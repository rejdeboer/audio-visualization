...
impl Visualization {
    pub fn new() -> Self {
        let buffer = [0f32; BUFFER_SIZE];
        info!("{}", buffer[0]);
        Visualization {
            smoothing_buffer: [0f32; BUFFER_SIZE],
        }
    }
    pub fn generate_vertices(&mut self, fft_spectrum: FftSpectrum, dt: f32) -> (Vec<Vertex>, Vec<u32>) {
        let mut vertices: Vec<Vertex> = Vec::new();
        let mut indices: Vec<u32> = Vec::new();
        let frequency_vertices = self.generate_frequency_vertices(fft_spectrum, dt);
    ...
    fn generate_frequency_vertices(&mut self, fft_spectrum: FftSpectrum, dt: f32) -> Vec<FrequencyVertex> {
        ...
        // apply_a_weighting(&mut vertices);
    ...
    fn apply_a_weighting(&self, vertices: &mut Vec<FrequencyVertex>) {
        for vertex in vertices {
            vertex.volume *= get_a_weighting(vertex.frequency);
        }
    }
    ...
    fn apply_normalised_positioning(&self, vertices: &mut Vec<FrequencyVertex>) {
        let max_vol = vertices
            .iter()
            .fold(0f32, |max, vertex| if max < vertex.volume { vertex.volume } else { max });
        let max_vol = 5000.;
    ...
    fn apply_smoothing(&mut self, vertices: &mut Vec<FrequencyVertex>, dt: f32) {
